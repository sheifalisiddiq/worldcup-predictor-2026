...
var REF_PARAM = null;
try {
  REF_PARAM = new URLSearchParams(window.location.search).get('ref');
} catch (e) {}
...
function App() {
          ...
          if (!data) {
            ...
            if (REF_PARAM && REF_PARAM !== user.uid) {
              try {
                ...
                console.error('Referral write error:', e);
              }
            }
          } else {
    ...
    async function loadMatches(attempt = 0) {
      try {
        ...
        if (data.matches && data.matches.length) {
          ...
          try {
            localStorage.setItem(LS.matches, JSON.stringify(data.matches));
          } catch (e) {}
          return;
        }
        ...
        if (attempt < 2) {
          setTimeout(() => {
            if (!cancelled) loadMatches(attempt + 1);
          }, 1200 * (attempt + 1));
          return;
        }
        setMatchesLoading(false);
  ...
  useEffect(() => {
    ...
    let hasRef = false;
    try {
      hasRef = new URLSearchParams(window.location.search).has('ref');
    } catch (e) {}
    if (hasRef) {
      try {
        window.history.replaceState({}, '', window.location.pathname);
...
function MobileContent({
  ...
  function onTouchMove(e) {
    ...
    if (!engaged.current) {
      if (Math.abs(dx) > 12 && Math.abs(dx) > Math.abs(dy) * 1.2) engaged.current = true;else if (Math.abs(dy) > 12) {
        start.current = null;
        return;
      } // vertical scroll → let it be
      else return;
    }
    lastDx.current = dx;
    setDrag({
      dx,
      target: targetFor(dx),
      progress: Math.min(1, Math.abs(dx) / COMMIT),
      dir: dx < 0 ? 1 : -1
    });
  }
  function onTouchEnd() {
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      height: '100%',
      transform: peek ? `translateX(${peek}px)` : 'none',
      transition: drag ? 'none' : 'transform .2s cubic-bezier(.34,1.1,.64,1)'
    }
  }, product), targetTab && /*#__PURE__*/React.createElement("div", {
    style: {
      position: 'absolute',
      top: '50%',
      ...(drag.dir === 1 ? {
  ...
  }, /*#__PURE__*/React.createElement(targetTab.Ico, {
    size: 22
  }), /*#__PURE__*/React.createElement("span", {
    className: "heavy",
    style: {
